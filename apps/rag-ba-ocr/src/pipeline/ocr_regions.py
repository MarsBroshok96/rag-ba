...
from src.common.project_paths import OCR_LAYOUT_ROOT, REPO_ROOT, ROOT_LAYOUT_ROOT
...
def _iter_layout_jsons(layout_root: Path) -> list[Path]:
    """
    layout_root ожидается как: <repo>/layout/<doc_id>/
...
def main() -> None:
    # --- input from root app (same repository) ---
    repo_root = REPO_ROOT
    in_layout_root = ROOT_LAYOUT_ROOT.resolve()

    assert in_layout_root.exists(), f"Input layout not found: {in_layout_root}"

    # --- output in rag-ba-ocr ---
    out_root = OCR_LAYOUT_ROOT
    ...
    for doc_dir in doc_dirs:
        ...
        for layout_json_path in layout_jsons:
            ...
            if not page_png_path.is_absolute():
                ...
                page_png_path = (repo_root / page_png_path).resolve()
